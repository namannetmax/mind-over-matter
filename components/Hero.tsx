  ...
  return (
    <section className="relative h-screen min-h-[800px] flex items-center bg-gym-hero px-6 lg:px-24 overflow-hidden">
      <div className="max-w-4xl z-10 space-y-6">
        <span className="text-red-700 font-bold tracking-[0.5em] text-xs uppercase block reveal">100% BESPOKE PERFORMANCE</span>
        ...
        <p className="max-w-xl text-lg md:text-xl text-white/60 font-light leading-relaxed reveal" style={{ transitionDelay: '0.4s' }}>
          No generic programs. No template routines. Just elite, one-on-one personal training engineered specifically for your biology.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 pt-8 reveal" style={{ transitionDelay: '0.6s' }}>
          <button className="bg-red-700 text-white px-12 py-5 text-sm tracking-[0.2em] font-bold hover:bg-white hover:text-black transition-all duration-300">
            BOOK CONSULTATION
          </button>
          <button className="border border-white/20 text-white px-12 py-5 text-sm tracking-[0.2em] font-bold hover:bg-white hover:text-black transition-all duration-300">
            OUR APPROACH
          </button>
        </div>
      </div>
